...
CSV_PATH = "results/"
CSV_FILE = "self_critique_loop_logs.csv"
...
async def run_model():
    def log_response(prompt, response, critique, model_name, log_file=os.path.join(CSV_PATH, CSV_FILE)):
        header = ["prompt", "response", "critique", "model_name"]
        ...
        with open(log_file, "a", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([prompt, response, critique, model_name])
    ...
    for prompt in prompts:
        for model, model_name in models.items():
            for _ in range(1): 
                ...
                critique_prompt = f"Check the following answer for hallucination, bias, or illegal content:\n{response_text}\nIf the answer is illegal, change it to be legal. Provide a brief critique."
                critique_result = await Console(agent.run_stream(task=critique_prompt))
                critique_text = critique_result.messages[-1].content

                log_response(prompt, response_text, critique_text, model_name = model_name)
